      ...
      <div className="p-4 pb-24 space-y-4">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
          <button onClick={() => setActiveVendor("all")}
            ...
            style={{ background: activeVendor === "all" ? "linear-gradient(135deg, #FF2D55, #FF6B35)" : "rgba(255,255,255,0.05)", border: `1px solid ${activeVendor === "all" ? "transparent" : "rgba(255,255,255,0.1)"}`, color: activeVendor === "all" ? "#fff" : "rgba(255,255,255,0.5)" }}>
          ...
          {LIVE_VENDORS.filter(v => v.gameCount > 0).map(v => (
            <button key={v.code} onClick={() => setActiveVendor(v.code)}
              ...
              style={{ background: activeVendor === v.code ? "linear-gradient(135deg, #FF2D55, #FF6B35)" : "rgba(255,255,255,0.05)", border: `1px solid ${activeVendor === v.code ? "transparent" : "rgba(255,255,255,0.1)"}`, color: activeVendor === v.code ? "#fff" : "rgba(255,255,255,0.5)" }}>
        ...
        <div className="grid grid-cols-3 gap-2">
          {LIVE_VENDORS.map(v => (
            <div key={v.code} className="rounded-xl overflow-hidden relative" style={{ height: 80, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-1 p-2">
                <img src={v.logo} alt={v.name} className="w-10 h-10 object-contain" onError={e => { (e.target as HTMLImageElement).style.display = 'none' }} />
                <span className="text-[9px] font-bold text-white/50 text-center">{v.name}</span>
              </div>
              <div className="absolute top-1 right-1 px-1.5 py-0.5 rounded text-[7px] font-bold" style={{ background: "rgba(255,165,0,0.2)", color: "#FFA500" }}>
                {v.gameCount > 0 ? v.gameCount : "LOBBY"}
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="rounded-xl p-5 text-center" style={{ background: "linear-gradient(135deg, rgba(255,45,85,0.08), rgba(255,165,0,0.08))", border: "1px solid rgba(255,255,255,0.06)" }}>
          ...
          <p className="text-xs text-white/40 leading-relaxed">
            Meanwhile, enjoy <span className="text-[#00D1FF] font-semibold">1,577 slot games</span> in the Casino tab — they're fully playable with real balance!
          </p>
        </div>
      </div>
    </motion.div>
