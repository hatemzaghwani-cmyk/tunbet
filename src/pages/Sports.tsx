...
export default function Sports() {
  ...
  const toggleSel = (m: OddsMatch, mk: string, sel: string, odds: number) => {
    ...
    // Keep the slip collapsed to a floating button — the user opens it when ready.
  };
  ...
  return (
    <div className="pb-32 px-3 pt-2">
      ...
      {/* Floating circular Bet Slip button (FAB) — always reachable, never covers matches */}
      {slip.length > 0 && !showSlipPanel && (
        ...
        >
          <Ticket className="w-6 h-6" style={{ color: "#020408" }} />
          <span
            className="absolute -top-1.5 -right-1.5 min-w-[22px] h-[22px] px-1 rounded-full flex items-center justify-center text-[11px] font-black"
            style={{ background: "#FF2D55", color: "#fff", border: "2px solid #020408" }}
          >
            {slip.length}
          </span>
        </button>
    ...
    <div className="rounded-2xl overflow-hidden shadow-2xl flex flex-col max-h-[75vh]"
      ...
      <div className="px-3 py-2 flex items-center justify-between flex-shrink-0" style={{ background: "rgba(0,209,255,0.08)" }}>
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setShow(false)} className="p-1 rounded-lg" style={{ background: "rgba(255,255,255,0.06)" }} aria-label="Minimize">
